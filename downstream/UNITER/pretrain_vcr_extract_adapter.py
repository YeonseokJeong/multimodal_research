...
from data import (TokenBucketSampler,
                  ...
                  MlmDatasetForVCRDC, mlm_collate_for_vcr_dc)

from model.adapter_pretrain_vcr import UniterAdapterForPretrainingForVCR
...
from adapter.src.transformers.adapters.configuration import AdapterConfig, PfeifferConfig
...
def build_mlm_dataset(txt_db, img_db_gt, img_db, is_train, opts):
    if is_train:
        collate_fn = mlm_collate_for_vcr_dc
        datasets = [MlmDatasetForVCRDC(t, i_gt, i)
                    for t, i_gt, i in zip(txt_db, img_db_gt, img_db)]
        dataset = ConcatDatasetWithLens(datasets)
    else:
        collate_fn = mlm_collate_for_vcr_dc
        dataset = MlmDatasetForVCRDC(txt_db, img_db_gt, img_db)
...
def main(opts):
    ...
    model = UniterAdapterForPretrainingForVCR.from_pretrained(
    ...
    ### 1-1> adapter pretrain을 위한 add_adapter - likelihood
    '''
    model.add_adapter("likelihood", config = PfeifferConfig())
    model.train_adapter(["likelihood"])
    '''
    ### 밑에서 save도 수정하기!!!

    ### 1-2> adapter pretrain을 위한 add_adapter - causality
    '''
    model.add_adapter("causality", config = PfeifferConfig())
    model.train_adapter(["causality"])
    '''
    ### 밑에서 save도 수정하기!!!

    ### 2> adapter fusion
    
    model.load_adapter("likelihood_save", config=PfeifferConfig(), with_head=False)
    model.load_adapter("causality_save", config=PfeifferConfig(), with_head=False)
    
    model.train_adapter(["likelihood", "causality"])
    adapter_setup = [["likelihood", "causality"]]
    ...
    if global_step % opts.valid_steps != 0:
        ...
        validate(model, val_dataloaders)
        model_saver.save(model, global_step)
        # model.save_adapter("likelihood_save", "likelihood")
        # model.save_adapter("causality_save", "causality")

...
from data.vcr import VcrDcDataset, vcr_dc_collate, VcrDcEvalDataset, vcr_dc_eval_collate
from model.vcr import UniterForVisualCommonsenseReasoning, UniterForVisualCommonsenseReasoningDc
...
def main(opts):
    ...
    for txt_path, img_path in zip(opts.train_txt_dbs, opts.train_img_dbs):
        ...
        train_datasets.append(
            VcrDcDataset(qa_txt_db, img_db_gt=img_db_gt, img_db=img_db))
        train_datasets.append(
            VcrDcDataset(qar_txt_db, img_db_gt=img_db_gt, img_db=img_db))
    train_dataset = ConcatDatasetWithLens(train_datasets)
    train_dataloader = build_dataloader(train_dataset, vcr_dc_collate, True, opts)
    # val
    ...
    val_dataset = VcrDcEvalDataset(
        "val", val_txt_db, img_db=val_img_db, img_db_gt=val_img_db_gt)
    val_final_dataset = VcrDcEvalDataset(
        "test", val_txt_db, img_db=val_img_db, img_db_gt=val_img_db_gt)
    val_dataloader = build_dataloader(val_dataset, vcr_dc_eval_collate,
                                      False, opts)
    val_final_dataloader = build_dataloader(
        val_final_dataset, vcr_dc_eval_collate,
        False, opts)
    ...
    model = UniterForVisualCommonsenseReasoningDc.from_pretrained(

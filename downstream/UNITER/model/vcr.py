...
class UniterForVisualCommonsenseReasoningDc(UniterPreTrainedModel):
    """ Finetune UNITER for VCR
    """
    def __init__(self, config, img_dim):
        super().__init__(config, img_dim)
    ...
    def forward(self, batch, compute_loss=True):
        ...
        txt_type_ids = batch['txt_type_ids']
        # do-calculus #
        input_ids_dc = batch['input_ids_dc']
        position_ids_dc = batch['position_ids_dc']
        attn_masks_dc = batch['attn_masks_dc']
        gather_index_dc = batch['gather_index_dc']
        txt_type_ids_dc = batch['txt_type_ids_dc'] 
        ...
        sequence_output_dc, _ = self.uniter(input_ids_dc, position_ids_dc,
                                    img_feat, img_pos_feat,
                                    attn_masks_dc, gather_index_dc,
                                    output_all_encoded_layers=False,
                                    txt_type_ids=txt_type_ids_dc)
        pooled_output_dc = self.uniter.pooler(sequence_output_dc)
        rank_scores_dc = self.vcr_output(pooled_output_dc)

        rank_scores = (rank_scores + rank_scores_dc)/2
